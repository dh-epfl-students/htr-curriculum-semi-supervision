...
class Engine(object):
    ...
    def _run_iteration_output(self, batch_n, batch):
        ...
        with self.exception_catcher(batch):
            ...
            return batch_output, batch_target
        return None
    ...
    def _run_iteration(self, batch_n, batch, train_iterations=None):
        ...
        self._iterations += 1
        action_kwargs["train_iterations"] = train_iterations
    ...
    def must_compute_validation(self,train_iterations):
        if 0 <= train_iterations and train_iterations < self.total_nb_iterations//4:
            return train_iterations%2==0
        elif self.total_nb_iterations//4 <= train_iterations and train_iterations < self.total_nb_iterations//2:
            return train_iterations%4==0
        elif self.total_nb_iterations//4 <= train_iterations and train_iterations < 3*self.total_nb_iterations//4:
            return train_iterations%8==0
        elif 3*self.total_nb_iterations//4 <= train_iterations:
            return train_iterations%16==0
            
    def _run_epoch(self, train_iterations=None):
        print("Lengthof dataset:", len(self._data_loader.dataset))
        if self.use_cl or self.use_baseline:
            if train_iterations!= None and not self.must_compute_validation(train_iterations):
                return None
        ...
        for it, batch in enumerate(batch_iterator, 1):
            if self._must_stop:
                break
            self._run_iteration(it, batch, train_iterations)
        else:
            self._epochs += 1
            self._call_hooks(EPOCH_END, epoch=self._epochs, train_iterations=train_iterations)
