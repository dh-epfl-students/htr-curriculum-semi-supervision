...
class HTRExperiment(Experiment):
    def __init__(
        ...
        if self.use_semi_supervised:
            self._va_engine.add_hook(EPOCH_END, self._check_validation_semi_supervised)
            self._va_engine.add_hook(EPOCH_END, self._tr_engine.compute_semi_supervision)

        if self._va_engine:
            self._va_cer = SequenceErrorMeter()
            self._va_wer = SequenceErrorMeter()

            self._va_engine.add_hook(EPOCH_END, self._update_cer_ls)
    ...
    @action
    def _check_validation_semi_supervised(self):
        if self._va_cer.value <= 0.50:
            self._tr_engine.start_semi_supervision()
    ...
    @action
    def _valid_update_meters(self, batch, batch_output, batch_target, epoch):
        ...
        _,_,_ = self._va_loss.value, self._va_cer.value, self._va_wer.value
        
        # Stop timer to avoid including extra costs
        self._va_timer.stop()

    @action
    def _update_cer_ls(self,train_iterations):
        if not train_iterations:
            self.epoch_va_cer += 1
            train_iterations = self.epoch_va_cer
            
        self.va_cer_dict[train_iterations] = self._va_cer.value
        
    def _decode_output(self,  batch_output):
        batch_decode = self._ctc_decoder(batch_output)
        return batch_decode
