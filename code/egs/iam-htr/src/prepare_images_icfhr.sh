...
for p in $(seq ${#partitions[@]}); do
  ...
  [ ! -d "data_icfhr/original/$partition" ] &&
  echo "ERROR: No data found for partition \"$partition\"!" >&2 &&
  exit 1;


  img_dir="data_icfhr/imgs/${partition}";
  img_resize_dir="data_icfhr/imgs/${partition}_h${resize_height}";
  ...
  [[ "$overwrite" = false && "$actual_count" -eq "${expected_count[p-1]}" ]] || {
    ...
    for f in $(find "data_icfhr/original/$partition" -name "*.png"); do
