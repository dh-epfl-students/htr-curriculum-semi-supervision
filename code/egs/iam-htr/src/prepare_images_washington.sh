...
for p in $(seq ${#partitions[@]}); do
  ...
  [ ! -d "data_washington/original/$partition" ] &&
  echo "ERROR: No data found for partition \"$partition\"!" >&2 &&
  exit 1;


  img_dir="data_washington/imgs/${partition}";
  img_resize_dir="data_washington/imgs/${partition}_h${resize_height}";
  ...
  [[ "$overwrite" = false && "$actual_count" -eq "${expected_count[p-1]}" ]] || {
    ...
    for f in $(find "data_washington/original/$partition" -name "*.png"); do
